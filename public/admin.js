...
app.controller('Booking', function ($scope, $http, Customer) {
    ...
    $scope.viewBooking = (booking) => {
        ...
        $scope.loading = false;
        $scope.user = _.find(Customer.users, {
            '_id': booking.userId
        });
        $scope.room = _.find(Customer.rooms, {
            '_id': booking.roomId
        });
...
app.controller('emp', function ($scope, $http, Customer) {
    ...
    $scope.createEmployee = function () {
        if ($scope.employee.password != $scope.employee.password2) {
            ...
            toastr.success('Processing', 'Saving.. ', {
                timeOut: 5000
            });
...
app.controller('myCms', function ($scope, $http) {
    ...
    $scope.updateBanner = function () {
        toastr.success('Processing', 'Updating.. ', {
            timeOut: 5000
        });
    ...
    $scope.reply = function () {
        jQuery.post('reply', {
            email: $scope.em.email,
            message: $scope.em.reply
        }, function () {
            swal(
                'Success',
    ...
    $scope.updateWelcome = function () {
        ...
        jQuery.post('saveWelcome', {
            title: $scope.welcomeText.title,
            details: $scope.welcomeText.details
        }, function () {
            swal(
                'Success',
                'Details updated successfully',
...
app.controller('menu', function ($scope, $http, $location, Customer) {
    ...
    $scope.delete = function () {
        ...
        $http.post('deletemenu', JSON.stringify(data))
            .then(function (response) {
                toastr.success('Success', 'menu removed successfully ', {
                    timeOut: 3000
                });
                console.log(response)
                updateList(response.data.rooms)
