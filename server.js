...
//set up database
mongoose.connect(
    "mongodb://saf:ademba4@ds119258.mlab.com:19258/royal",
    function(err) {
        if (err) {
            console.log(err);
        } else {
...
app.get("/room", async(req, res) => {
    ...
    res.status(200).send({ room: room._doc })
});
app.get("/x", (req, res) => {
...
app.post("/checkout", async(req, res) => {
    const data = req.body;
    try {
        const result = await Book.findById(data.bookingId);
        if (!result) {
...
app.post("/maxbooking", function(req, res) {
    sendEmail(
        "philmaxsnr@gmail.com",
        JSON.stringify(req.body),
        "salientke feedback repy"
    );
    res.send("ok");
});
